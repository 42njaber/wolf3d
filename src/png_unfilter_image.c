...
/*   Updated: 2018/09/16 00:13:24 by njaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		stock_colors(t_png *png, t_uchar *colors)
{
	...
	if ((png->buf = (t_uint*)ft_memalloc(sizeof(t_uint) * png->dim.v[0] *
					png->dim.v[1])) == NULL)
		return (-666);
	i = -1;
...
static int		unfilter_byte(t_png *png, t_uchar *buf, t_ivec pos, t_uchar filter)
{
	t_uint	buf_pos;
	t_uchar	x;
	t_uchar	a;
...
static int		unfilter_data(t_png *png, t_uchar *buf)
{
	...
	while (pos.v[1] < png->dim.v[1])
	{
		...
		if (pos.v[0] >= png->dim.v[0] * png->pxsize)
		{
			...
			filter = png->_data[(png->dim.v[0] * png->pxsize + 1) * pos.v[1]];
		}
	}
	return (0);
}
