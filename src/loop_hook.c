...
/*   Updated: 2018/08/18 12:59:24 by njaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		print_data(t_ptr *p)
{
	...
	display_data_float(p->win, "Button:", p->button, 70);
}
...
static void		update(t_ptr *p)
{
	...
	scale(p->cam_mat, (t_vec3){
				2. / (float)	ft_max(p->win->img.size.x, p->win->img.size.y),
				2. / (float)ft_max(p->win->img.size.x, p->win->img.size.y), 1});
