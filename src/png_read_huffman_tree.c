...
/*   Updated: 2018/09/15 21:36:17 by njaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		read_clens(t_png *png, t_uchar *stream, t_uint *pos, t_ushort max)
{
	t_uchar	clens[19];
	t_uchar	order[19];
	int		i;
	...
	if (max > 19)
		return (-6);
	if ((*pos + max * 3 - 1) / 8 >= png->_zlib_len - 6)
		return (-2);
...
static int		copy_all_codes(t_png *png, t_uint *pos, t_uchar *lens, t_ushort max)
{
	t_uchar	*stream;
	int		i;
	t_uchar	len;
	...
	while (i < max)
	{
		...
		{
			if ((*pos + (val == 18 ? 7 : val - 14)) / 8 >= png->_zlib_len - 6)
				return (-2);
